...
export const parseResponse = async <T>(response: Response): Promise<ResponseWrapper<T>> => {
  if (!response.ok) {
    throw response
  }
...
const parseJson = async <T>(response: Response): Promise<ResponseWrapper<T>> => {
  try {
    return {
...
const parseText = async <T>(response: Response): Promise<ResponseWrapper<T>> => {
  try {
    return {
...
const parseBoolean = (response: ResponseWrapper<string>): ResponseWrapper<string | boolean> => {
